...
const Amazon_Contract_Address = "0x971A51bA23778D46E443b82829B90F8290712Dcb";
const Amazon_NFT_Address = "0x5412044Ee33a4F9a1594a080215555BcDbfD997f";
const Amazon_USD_Address = "0x62dd1012142D23b37A9515b8B59881F6FEAd807B";

const Amazon_USD_ABI = [
	{
		"inputs": [],
...
async function main() {
	...
	AmazonNFT = new ethers.Contract(Amazon_NFT_Address, Amazon_NFT_ABI, signer);
	AmazonUSD = new ethers.Contract(Amazon_USD_Address, Amazon_USD_ABI, signer);

	let cBalance = await AmazonMPContract.getBalance();
	dBalance.setValue(cBalance);
...
const deposit = async () => {
	if (dDamount.getValue() < 0.01){
		...
		$$("win1").show();
		let data = await fetch('https://min-api.cryptocompare.com/data/pricemulti?fsyms=ETH&tsyms=USD').then(response => response.json())
  		console.log(data["ETH"]["USD"])
		const transaction = await AmazonMPContract.deposit(parseInt(data["ETH"]["USD"]), { value: ethers.utils.parseEther(dDamount.getValue()) });
...
const withdraw = async() => {
	if (dWamount.getValue() <= 25){
		alert("MINIMUM WITHDRAWL AMOUNT 25 USD!");
	} else{
		$$("win1").show();
		let data = await fetch('https://min-api.cryptocompare.com/data/pricemulti?fsyms=ETH&tsyms=USD').then(response => response.json())
  		console.log(data["ETH"]["USD"])
		let cBalance = await AmazonMPContract.getBalance();
		const approveTrans = await AmazonUSD.approve(Amazon_Contract_Address, cBalance);
		await approveTrans.wait();
		const transaction = await AmazonMPContract.withdraw(parseInt(data["ETH"]["USD"]), dWamount.getValue());
		await transaction.wait();
		$$("win1").hide();
		main();
	}
...
const buyItem = async(_id, _price) => {
	...
	if(dBalance.getValue() < _price){
		...
		const transaction = await AmazonMPContract.buyItem(_id, _price);
...
const addItem = async(_ipfsCID, _category, _price, _instock) => {
	$$("win2").disable();
	const nftTokenId = await AmazonNFT.createToken(_ipfsCID);
	await nftTokenId.wait();
	const transaction = await AmazonMPContract.addItem(nftTokenId, _price, _category, _instock);
...
const editItem = async(_id, _price, _instock) => {
	$$("win3").disable();
	const transaction = await AmazonMPContract.editItem(_id, _price, _instock);
...
const getItems = async () => {
	...
	var _itemData = [];
	for(i = 0; i < num; i++){
		let metaData = await fetch('https://' + itemData[i][6] + '.ipfs.w3s.link/meta.json').then(response => response.json());
		_itemData.push({ id:itemData[i][0], title:metaData['title'], info:metaData['info'], cat:itemData[i][5], price:itemData[i][4]});
	}
...
const getMyItems = async () => {
	...
	for(i = 0; i < num; i++){
		let metaData = await fetch('https://' + itemData[i][6] + '.ipfs.w3s.link/meta.json').then(response => response.json());
		let ownerInfo = await AmazonNFT.ownerOf(itemData[i][2]);
		_itemData.push({ id:itemData[i][0], title:metaData['title'], info:((metaData['info']).replace(/[\n\r]/g, ' ')).replace('  ', ' '), cat:itemData[i][5], price:itemData[i][4], instock:((ownerInfo == Amazon_Contract_Address) ? true : false)});
